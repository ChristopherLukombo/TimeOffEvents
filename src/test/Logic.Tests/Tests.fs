...
let shouldfindAccruedHolidaysToDaysWhenIsEmpty =
  testList "tests findAccruedHolidaysToDays" [
    test "A request to findAccruedHolidaysToDays when is empty" {
      ...
      Expect.isTrue (Logic.findAccruedHolidaysToDays userRequestsState userId consultationDate = 0) "Requests should findAccruedHolidaysToDays"
    }
  ]
...
let shouldfindAccruedHolidaysToDaysWhenIsOutOfBounds =
  testList "tests findAccruedHolidaysToDays" [
    test "A request to findAccruedHolidaysToDays when is out of bounds" {
      ...
      // Then
      Expect.isTrue (Logic.findAccruedHolidaysToDays userRequestsState userId consultationDate = 0)
  ...
  testList "tests findAccruedHolidaysToDays" [
    test "A request to findAccruedHolidaysToDays when is not empty" {
      ...
      let request = {
        UserId = "jdoe"
        RequestId = Guid.NewGuid()
        Start = { Date = (dateProvider :> IDateProvider).DateTime 2019 5 1; HalfDay = AM }
        End = { Date = (dateProvider :> IDateProvider).DateTime 2019 6 30; HalfDay = PM } }
      let userRequestsState =
          [(Guid.NewGuid(), Logic.Validated request)]
          |> Map.ofList;
      let userId = "jdoe"
      let consultationDate = (dateProvider :> IDateProvider).CurrentDate
...
[<Tests>]
let shouldfindAccruedHolidaysToDaysWhenIsNotEmptyAndNotFullMonth =
  testList "tests findAccruedHolidaysToDays" [
    test "A request to findAccruedHolidaysToDays when is not empty not full month" {
      // Given
      let dateProvider = DateProvider()
      let request = {
        UserId = "jdoe"
        RequestId = Guid.NewGuid()
        Start = { Date = (dateProvider :> IDateProvider).DateTime 2019 5 1; HalfDay = AM }
        End = { Date = (dateProvider :> IDateProvider).DateTime 2019 7 10; HalfDay = PM } }
      let userRequestsState =
          [(Guid.NewGuid(), Logic.Validated request)]
          |> Map.ofList;
      let userId = "jdoe"
...
let shouldfindRemainingHolidaysFromLastYearWhenIsNotEmpty =
  testList "tests findRemainingHolidaysFromLastYear" [
    test "A request to findRemainingHolidaysFromLastYear when is not empty" {
      ...
      // Then
      Expect.isTrue (Logic.findRemainingHolidaysFromLastYear userRequestsState userId consultationDate <> 0)
  ...
  testList "tests findRemainingHolidaysFromLastYear" [
    test "A request to findRemainingHolidaysFromLastYear when is out of bounds" {
      ...
      let request = {
        ...
        End = { Date = (dateProvider :> IDateProvider).DateTime 2020 12 27; HalfDay = PM } }
      let userRequestsState = Map.ofList [ (Guid.NewGuid(), Logic.Validated request) ]
      let userId = "jdoe"
      let consultationDate = (dateProvider :> IDateProvider).CurrentDate
    ...
    testList "tests findFutureHolidays" [
      test "A request to findFutureHolidays when is not empty" {
        ...
        let request = {
          ...
          End = { Date =  (dateProvider :> IDateProvider).DateTime 2019 12 28; HalfDay = PM } }
        let userRequestsState = Map.ofList [ (Guid.NewGuid(), Logic.PendingValidation request) ]
        let userId = "jdoe"
        let consultationDate = (dateProvider :> IDateProvider).CurrentDate
